 ...
 * designed to demonstrate a simulated operating system (SOS). File History: HW
 * 1 Stephen Robinson and Camden McKone, HW 2 Stephen Robinson and Nathan Brown
 * HW 3 Stephen Robinson and Connor Haas HW 4 --Unchanged--
 * 
 * @see RAM
 * @see SOS
 * @see Program
...
public class CPU implements Runnable {
    ...
    public static final int CLOCK_FREQ = 5; // CPU cycles between clock
                                            // interrupts
    ...
    /**
     * Stores a reference to the CPU's interupt controller
     */
    private InterruptController m_IC = null;

    /**
     * Elapsed CPU cycles in this simulation
     */
    ...
    // ----------------------------------------------------------------------
    /**
     * TrapHandler This interface should be implemented by the operating system
     * to allow the simulated CPU to generate hardware interrupts and system
     * calls.
     */
    ...
    /**
     * CPU ctor Intializes all member variables.
     */
    ...
    /**
     * registerTrapHandler allows SOS to register itself as the trap handler
     */
     ...
     * getTicks
     * 
     * @return number of ticks during this simulation
     */
    public int getTicks() {
        return m_ticks;
    }
     ...
     * addTicks
     * 
     * @param v value added to m_ticks to simulate operations
     */
    public void addTicks(int v) {
        m_ticks += v;
    }

    /**
     * regDump Prints the values of the registers. Useful for debugging.
     */
    public void regDump() {
    ...
    /**
     * printIntr Prints a given instruction in a user readable format. Useful
     * for debugging.
     * 
    ...
    /**
     * validMemory Determines if physical address respects BASE and LIM
     * registers.
     * 
    ...
    /**
     * pushStack Pushes a value to the stack.
     * 
    ...
    public void pushStack(int value) {
        if (!validMemory(m_registers[SP] + m_registers[BASE])) {
            ...
            m_TH.interruptIllegalMemoryAccess(m_registers[SP]
                    + m_registers[BASE]);
        }
    ...
    /**
     * run Start the CPU simulation. Exits only on chrash or exit trap.
     */
    public void run() 
    {
        while (true) {
            ...
            // Proceed to next clock cycle and check for clock interrupt
            m_ticks++;
            ...
            if (!validMemory(m_registers[BASE] + m_registers[PC])) {
                m_TH.interruptIllegalMemoryAccess(m_registers[BASE] 
                        + m_registers[PC]);
                return;
            }
    ...
    /**
     * checkForIOInterrupt Checks the databus for signals from the interrupt
     * controller and, if found, invokes the appropriate handler in the
     * operating system.
     */
